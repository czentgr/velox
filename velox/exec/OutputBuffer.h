...
struct PendingRead {
  ...
  DataConsumerActiveCheckCallback aliveCheck;
  int64_t sequence{0};
  uint64_t maxBytes;

  PendingRead(
      const DataAvailableCallback& _callback,
      const DataConsumerActiveCheckCallback& _aliveCheck,
      int64_t _sequence,
      uint64_t _maxBytes)
      : callback(_callback),
        aliveCheck(_aliveCheck),
        sequence(_sequence),
        maxBytes(_maxBytes) {}

  void setEmptyResults() {
    if (callback) {
      callback({}, sequence, {});
    }
  }
  ...
  std::string toString() {
    return fmt::format(
        "[callback: {} aliveCheck: {} sequence: {}, maxBytes: {}]",
        callback ? "registered" : "",
        aliveCheck ? "registered" : "",
        sequence,
        maxBytes);
  }
};
...
class ArbitraryBuffer {
 public:
  ...
  bool empty() const {
    return pages_.withRLock([](auto& pages) {
      return pages.empty() || (pages.size() == 1 && pages.back() == nullptr);
    });
  }
  ...
  bool hasNoMoreData() const {
    return pages_.withRLock([](auto& pages) {
      return !pages.empty() && (pages.back() == nullptr);
    });
  }
 ...
 private:
  folly::Synchronized<std::deque<std::shared_ptr<SerializedPageBase>>> pages_;
...
class DestinationBuffer {
 public:
  ...
  struct Stats {
    ...
    Stats(
        bool _finished,
        int64_t _bytesBuffered,
        int64_t _rowsBuffered,
        int64_t _pagesBuffered,
        int64_t _bytesSent,
        int64_t _rowsSent,
        int64_t _pagesSent)
        : finished(_finished),
          bytesBuffered(_bytesBuffered),
          rowsBuffered(_rowsBuffered),
          pagesBuffered(_pagesBuffered),
          bytesSent(_bytesSent),
          rowsSent(_rowsSent),
          pagesSent(_pagesSent) {}
  ...
  //  void enqueue(std::shared_ptr<SerializedPageBase> newData);
  void enqueue(
  ...
  /// 'dataAvailableCallback_' is not null. Otherwise, it does nothing. This
  /// only used by arbitrary output type when enqueue new data.
  void loadDataIfNecessary(ArbitraryBuffer* arbitraryBuffer);
  ...
  void addPages(const std::vector<std::shared_ptr<SerializedPageBase>>& newPages);
  ...
  std::shared_ptr<PendingRead> pendingRead();
 ...
 private:
  ...
  void processReadLocked(
      const std::vector<std::shared_ptr<SerializedPageBase>>& data,
      uint64_t maxBytes,
      int64_t sequence,
      const ArbitraryBuffer* arbitraryBuffer,
  ...
  folly::Synchronized<std::vector<std::shared_ptr<SerializedPageBase>>> data_;
  // The sequence number of the first in 'data_'.
  std::atomic<int64_t> sequence_ = 0;
  std::atomic<bool> finished_ = false;
  std::atomic<int64_t> bytesBuffered_{0};
  std::atomic<int64_t> rowsBuffered_{0};
  std::atomic<int64_t> pagesBuffered_{0};
  std::atomic<int64_t> bytesSent_{0};
  std::atomic<int64_t> rowsSent_{0};
  std::atomic<int64_t> pagesSent_{0};

  std::shared_ptr<PendingRead> pendingRead_;
};
...
class OutputBuffer {
 ...
 private:
  ...
  void updateStatsWithEnqueuedPage(int64_t pageBytes, int64_t pageRows);

  void updateStatsWithFreedPages(int numPages, int64_t pageBytes);

  void updateTotalBufferedBytesMs();

  int64_t getAverageBufferTimeMs();
  ...
  void enqueueBroadcastOutput(std::unique_ptr<SerializedPageBase> data);

  void enqueueArbitraryOutput(std::unique_ptr<SerializedPageBase> data);

  void enqueuePartitionedOutput(
      int destination,
      std::unique_ptr<SerializedPageBase> data);
  ...
  FOLLY_ALWAYS_INLINE uint64_t buffersSize() {
    return buffers_.withRLock([&](auto& buffers) { return buffers.size(); });
  }
  ...
  std::atomic<int32_t> nextArbitraryLoadBufferIndex_{0};
  // One buffer per destination.
  folly::Synchronized<std::vector<std::unique_ptr<DestinationBuffer>>> buffers_;
  ...
  std::atomic<uint32_t> numDrivers_{0};
  ...
  std::atomic<bool> noMoreBuffers_{false};
  ...
  std::atomic<int64_t> bufferedPages_{0};
  // The total number of output bytes, rows and pages.
  std::atomic<uint64_t> numOutputBytes_{0};
  std::atomic<uint64_t> numOutputRows_{0};
  std::atomic<uint64_t> numOutputPages_{0};
  ...
  folly::Synchronized<std::vector<ContinuePromise>> promises_;
  ...
  std::atomic<uint32_t> numFinished_{0};
  // When this reaches buffers_.size(), 'this' can be freed.
  std::atomic<int> numFinalAcknowledges_ = 0;
