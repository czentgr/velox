...
using ExpressionRewriteFunc = std::function<core::TypedExprPtr(
...
class ExpressionRewrite {
 public:
  ExpressionRewrite(std::string_view name, ExpressionRewriteFunc rewriteFunc)
      : name_(name), rewriteFunc_(rewriteFunc){};

  core::TypedExprPtr operator()(
      const core::TypedExprPtr& expr,
      const std::shared_ptr<core::QueryCtx>& queryCtx,
      memory::MemoryPool* pool) const {
    return rewriteFunc_(expr, queryCtx, pool);
  };
 ...
 private:
  ...
  ExpressionRewriteFunc rewriteFunc_;
};
