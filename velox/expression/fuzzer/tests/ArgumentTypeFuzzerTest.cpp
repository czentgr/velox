...
TEST_F(ArgumentTypeFuzzerTest, concreteSignature) {
...

  {
    ...
    testFuzzingSuccess(signature, BIGINT(), {VARCHAR_N(10)});
  }

  {
    ...
    testFuzzingSuccess(signature, VARCHAR(), {VARCHAR_N(10)});
  }
}
...
TEST_F(ArgumentTypeFuzzerTest, signatureTemplate) {
...

  {
    ...
    verifyArgumentTypes(VARCHAR_N(10), ARRAY(VARCHAR_N(10)));
  }
...
TEST_F(ArgumentTypeFuzzerTest, fuzzVarcharArgumentTypes) {
  ...
  EXPECT_LE(getVarcharNLength(*argTypes[0]), kDefaultMaxVarcharLength);
  ...
  ASSERT_EQ(1, argTypes.size());
  EXPECT_TRUE(argTypes[0]->isVarchar());
  EXPECT_EQ(10, getVarcharNLength(*argTypes[0]));
  ...
  ASSERT_EQ(1, argTypes.size());
  EXPECT_TRUE(argTypes[0]->isVarchar());
  EXPECT_EQ(200, getVarcharNLength(*argTypes[0]));
  ...
  argTypes = fuzzArgumentTypes(*signature, VARCHAR_N(10));
  ASSERT_EQ(3, argTypes.size());
  EXPECT_EQ(VARCHAR_N(10)->toString(), argTypes[0]->toString());
  EXPECT_EQ(VARCHAR_N(10)->toString(), argTypes[1]->toString());
  EXPECT_EQ(VARCHAR_N(10)->toString(), argTypes[2]->toString());
  ...
  argTypes = fuzzArgumentTypes(*signature, VARCHAR_N(30));
  ASSERT_EQ(2, argTypes.size());
  EXPECT_EQ(VARCHAR_N(30)->toString(), argTypes[0]->toString());
  EXPECT_EQ(VARCHAR_N(20)->toString(), argTypes[1]->toString());
  ...
  EXPECT_LE(getVarcharNLength(*argTypes[0]), kDefaultMaxVarcharLength);
  EXPECT_LE(getVarcharNLength(*argTypes[1]), kDefaultMaxVarcharLength);
...
TEST_F(ArgumentTypeFuzzerTest, fuzzVarcharReturnType) {
  ...
  returnType = fuzzReturnType(*signature);
  EXPECT_TRUE(returnType->isVarchar());
  EXPECT_LE(getVarcharNLength(*returnType), kDefaultMaxVarcharLength);
  ...
  returnType = fuzzReturnType(*signature);
  EXPECT_TRUE(returnType->isVarchar());
  EXPECT_EQ(VARCHAR_N(200)->toString(), returnType->toString());
  ...
  // Return type can only be VARCHAR_N(100).
  signature = exec::FunctionSignatureBuilder()
                  .integerVariable("x")
  ...
  returnType = fuzzReturnType(*signature);
  EXPECT_EQ(VARCHAR_N(100)->toString(), returnType->toString());
  ...
  // Return type can only be VARCHAR_N(1).
  signature = exec::FunctionSignatureBuilder()
  ...
  returnType = fuzzReturnType(*signature);
  EXPECT_EQ(VARCHAR_N(1)->toString(), returnType->toString());
}
