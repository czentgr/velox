...
void registerGeometryType() {
    GEOMETRY()->castToVartext = [](const OGRGeometry* geometry) -> std::string {
    return GEOMETRY()->geometryToText(geometry);
  };

    GEOMETRY()->castFromVartext = [](const std::string& wkt) -> std::shared_ptr<OGRGeometry> {
    return GEOMETRY()->textToGeometry(wkt);
  };
