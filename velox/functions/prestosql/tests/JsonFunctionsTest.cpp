 ...
 protected:
  ...
  std::tuple<VectorPtr, VectorPtr, VectorPtr> makeVectors(
    ...
    return {
        ...
        makeNullableFlatVector<StringView>({s}, VARCHAR_N(varcharNLength))};
  }
  ...
  std::optional<bool> jsonArrayContains(
    ...
    if constexpr (std::is_same_v<T, std::string>) {
      ...
      auto valueVector =
          makeNullableFlatVector<T>({value}, VARCHAR_N(kValueLength + 10));
      auto varcharNResult = evaluateOnce<bool>(
          "json_array_contains(c0, c1)",
  ...
  std::optional<int64_t> jsonSize(
    ...
    // VARCHAR_N requires length > 0; for empty paths fall back to VARCHAR for
    // the bounded variant.
    auto pathVectorVarcharN = path.empty()
        ? makeFlatVector<std::string>({path}, VARCHAR())
        : makeFlatVector<std::string>({path}, VARCHAR_N(path.size()));
