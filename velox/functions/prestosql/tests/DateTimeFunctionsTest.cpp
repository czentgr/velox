...
TEST_F(DateTimeFunctionsTest, dateTruncTimeStampWithTimezoneStringForWeek) {
  ...
  const auto kFormatType = VARCHAR_N(kDateTimeFormat.size());
  const auto evaluateDateTruncFromStrings =
      [&](const std::string& truncUnit,
        ...
        const auto kUnitType = VARCHAR_N(truncUnit.size());
        const auto kInputType = VARCHAR_N(inputTimestamp.size());
...
TEST_F(DateTimeFunctionsTest, dateAddTimestampWithTimeZone) {
  ...
  constexpr std::string_view kDateTimeFormat = "YYYY-MM-dd+HH:mm:ssZZ";
  const auto kType = VARCHAR_N(100);
